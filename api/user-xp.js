...
========================= */
async function updateUserXP(db, userId, xpToAdd, course, topicIndex, score, quizScore) {
  ...
  // Calculate quiz score (can exceed 100%)
  // Use provided quizScore, or calculate from score, or add to existing totalScore
  let currentQuizScore = user.quizScore || user.totalScore || 0;
  let newQuizScore = currentQuizScore;
  
  if (quizScore !== undefined && quizScore !== null) {
    // If a specific quiz score is provided (can be >100)
    newQuizScore = quizScore;
  } else if (score !== undefined && score !== null) {
    // Add the assignment score to total quiz score (allowing >100)
    newQuizScore = currentQuizScore + (score || 0);
  } else {
    // Add XP as quiz score (XP can grow >100)
    newQuizScore = currentQuizScore + xpToAdd;
  }
  
  // Update user XP, level, and quiz score
  await userCollection.updateOne(
    ...
    { 
      $set: { 
        ...
        totalScore: newQuizScore, // Keep for backward compatibility
        lastActive: new Date()
      },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* =========================
       HANDLE POST REQUEST (ADD XP / UPDATE QUIZ SCORE)
    ========================= */
    if (req.method === "POST") {
      const { xpToAdd, course, topicIndex, score, quizScore } = req.body;
      
      if ((!xpToAdd || xpToAdd <= 0) && quizScore === undefined) {
        return res.status(400).json({ error: "Invalid XP amount to add or quizScore not provided" });
      }
      ...
      // Check if this topic was already completed (optional - can be disabled for retakes)
      ...
      // If quizScore is provided and it's greater than previous, update even if completed
      let shouldUpdate = true;
      let isRetake = false;
      
      if (existingProgress && quizScore !== undefined) {
        if (quizScore > (existingProgress.quizScore || 0)) {
          // Allow updating with higher quiz score
          isRetake = true;
          shouldUpdate = true;
        } else {
          shouldUpdate = false;
        }
      } else if (existingProgress && !quizScore) {
        return res.status(400).json({ 
          error: "Topic already completed", 
          message: "You have already earned XP for this topic. Use quizScore to update your score.",
      ...
      if (!shouldUpdate) {
        return res.status(400).json({
          error: "Score not higher than previous",
          message: "Your new score is not higher than your previous score.",
          currentScore: existingProgress?.quizScore || 0
        });
      }
      
      // Calculate XP to add (minimum 5 XP if updating score)
      let finalXpToAdd = xpToAdd || 5;
      if (quizScore !== undefined && (!xpToAdd || xpToAdd <= 0)) {
        // Award XP based on quiz score improvement
        const previousScore = existingProgress?.quizScore || 0;
        const improvement = Math.max(0, quizScore - previousScore);
        finalXpToAdd = Math.floor(improvement / 10) + 5;
      }
      
      // Update XP and quiz score
      const xpUpdateResult = await updateUserXP(db, user._id.toString(), finalXpToAdd, course, topicIndex, score, quizScore);
    ...
    /* =========================
       HANDLE GET REQUEST (FETCH XP & QUIZ SCORE)
    ========================= */
    if (req.method === "GET") {
      // Try ObjectId first
      if (userId && ObjectId.isValid(userId)) {
      ...
      // Get quiz score (can exceed 100%)
      const quizScore = userData.quizScore !== undefined ? userData.quizScore : (userData.totalScore || 0);
      
      // Calculate quiz percentage display (can exceed 100%)
      const quizPercentage = Math.round(quizScore);
      
      // Determine rank/title based on quiz score (now allows >100)
      let quizTitle = "Beginner 🌱";
      if (quizScore >= 500) {
        quizTitle = "God Mode 🛸✨";
      } else if (quizScore >= 300) {
        quizTitle = "Legend 👑🔥";
      } else if (quizScore >= 200) {
        quizTitle = "Elite ⚡💎";
      ...
      return res.status(200).json({
        ...
        canExceed100: true,
        maxQuizScore: "unlimited",
        message: quizScore >= 100 ? "🎉 Amazing! You've exceeded 100%!" : "Keep going to reach 100%+!"
      });
    }
...
export function calculateXPReward(score, maxScore = 100, baseXP = 50) {
  ...
  const percentage = Math.max(0, score);
  // XP scales with quality: 0-200% = 0-200 XP
...
export function getAchievements(xp, assignmentsCompleted, streak, quizScore) {
  const achievements = [];
  ...
  // New achievements for exceeding 100%
  if (quizScore >= 100) achievements.push({ id: "century_quiz", name: "Century Quiz", description: "Achieved 100% or more on quizzes", icon: "🎯" });
  if (quizScore >= 150) achievements.push({ id: "overdrive", name: "Overdrive", description: "Exceeded 150% quiz score", icon: "⚡" });
  if (quizScore >= 200) achievements.push({ id: "godlike", name: "Godlike", description: "Achieved 200%+ quiz score", icon: "👑" });
...
// Function to format quiz score for display (always shows as percentage)
export function formatQuizScore(quizScore) {
  const score = Math.round(quizScore || 0);
  return `${score}%`;
}

// Function to get color based on quiz score (supports >100)
export function getQuizScoreColor(quizScore) {
  const score = quizScore || 0;
  if (score >= 200) return "#FFD700"; // Gold
  if (score >= 150) return "#FFA500"; // Orange
