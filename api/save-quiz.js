...
/* =========================
   XP CALCULATION FOR ASSIGNMENTS
========================= */
function calculateAssignmentXP(score, percent, assignmentQuality = 0, isFirstAttempt = false) {
  let xpEarned = 0;
  
  // Base XP based on percentage (max 60 XP)
  const baseXP = Math.floor((percent / 100) * 60);
  xpEarned += baseXP;
  ...
  if (assignmentQuality >= 85) {
    xpEarned += 15;
  }
  
  // Cap at 150 XP per assignment
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (providedScore !== undefined && providedPercent !== undefined) {
        ...
        gradeInfo = {
          letterGrade: finalPercent >= 90 ? "A" : finalPercent >= 80 ? "B" : finalPercent >= 70 ? "C" : "D",
      ...
      if (xpEarned === undefined) {
        ...
        xpEarned = calculateAssignmentXP(finalScore, finalPercent, assignmentQuality, isFirstAttempt);
      }
