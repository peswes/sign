...
function calculateXP(score, totalQuestions, percent, assignmentQuality = 0, isFirstAttempt = false, timeSpent = 0) {
  ...
  if (assignmentQuality > 0) {
    xpEarned += Math.floor(Math.min(assignmentQuality, 100) * 0.5); // Quality bonus up to 25 XP
  }
...
function calculateGrade(score, totalQuestions, reflections = [], followups = []) {
  ...
  finalPercentage = Math.min(finalPercentage, 100); // Hard cap at 100%
  
  return {
    percentage: Math.round(finalPercentage),
    letterGrade: finalPercentage >= 90 ? "A" : 
                  finalPercentage >= 80 ? "B" :
                  finalPercentage >= 70 ? "C" :
                  finalPercentage >= 60 ? "D" : "F",
    gradePoint: finalPercentage >= 90 ? 4.0 :
                finalPercentage >= 80 ? 3.0 :
                finalPercentage >= 70 ? 2.0 :
                finalPercentage >= 60 ? 1.0 : 0.0,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (course) {
        ...
        return res.json({
          ...
          totalXP: totalXP,
          topicStatus: processedResults.map(r => ({
            topicIndex: r.topicIndex,
            completed: r.status === "completed",
            score: Math.min(r.percent, 100),
            xpEarned: r.xpEarned
          }))
        });
      }
      ...
      // Calculate percent with hard cap at 100
      ...
      // Calculate completion percentage per course (capped at 100)
      Object.keys(courseStats).forEach(course => {
        const stats = courseStats[course];
        let percentage = stats.totalQuestions > 0 
          ? Math.round((stats.totalScore / stats.totalQuestions) * 100) 
          : 0;
        stats.percentage = Math.min(percentage, 100);
      });
      ...
      // Cap recent results percentages
    ...
    if (req.method === "POST") {
      ...
      ========================= */
      // Calculate grade based on quiz answers (will automatically cap at 100%)
      const gradeInfo = calculateGrade(score, totalQuestions, reflections, followUpAnswers);
      
      // Ensure client percent doesn't exceed 100
      const cappedClientPercent = Math.min(clientPercent, 100);
      
      // Calculate XP based on performance (using capped percent)
      ...
      const quizData = {
        ...
        percent: gradeInfo.percentage, // Already capped at 100
        answers: answers || [],
      ...
      return res.json({
        ...
        courseProgress: {
          ...
          percentage: Math.min(Math.round((completedTopics / totalTopicsForCourse) * 100), 100),
