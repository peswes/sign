...
/* =========================
   GET USER QUIZ PERCENTAGE
   Returns the most recent quiz percentage for the frontend display
========================= */
async function getUserQuizPercentage(db, userId) {
  // Find the most recent quiz result for this user
  const latestQuiz = await db.collection("quizResults")
    .find({ userId: userId })
    .sort({ completedAt: -1 })
    .limit(1)
    .toArray();
  ...
  return latestQuiz[0].percentage || 0;
}

/* =========================
   GET ALL QUIZ RESULTS
========================= */
async function getAllUserQuizzes(db, userId) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* =========================
       GET REQUESTS - Check assignment status & quiz score
    ========================= */
    if (req.method === "GET") {
      const { mode, course, topic } = req.query;

      // Special endpoint for quiz score display (used by the frontend component)
      if (mode === "quiz-score") {
        const quizPercentage = await getUserQuizPercentage(db, user._id);
        
        // Generate a remark based on percentage
        let remark = "";
        if (quizPercentage === 100) remark = "🎉 Perfect Score!";
        else if (quizPercentage >= 90) remark = "🏆 Excellent!";
        else if (quizPercentage >= 80) remark = "⭐ Great job!";
        else if (quizPercentage >= 70) remark = "💪 Very good!";
        else if (quizPercentage >= 60) remark = "📚 Good effort!";
        else if (quizPercentage >= 50) remark = "🌱 Keep going!";
        else if (quizPercentage >= 30) remark = "✨ Keep learning!";
        else if (quizPercentage > 0) remark = "📝 Try again!";
        else remark = "📝 Take a quiz to see your score";
        
        return res.json({
          success: true,
          quizPercentage: Math.round(quizPercentage),
          remark: remark,
          hasQuiz: quizPercentage > 0,
      ...
      if (mode === "quiz-status" && course) {
        ...
        return res.json({
          ...
          percentage: existingQuiz?.percentage || 0,
          completedAt: existingQuiz?.completedAt || null
        });
      }

      // Get all quizzes for the user (for dashboard)
      if (mode === "all-quizzes") {
        const allQuizzes = await getAllUserQuizzes(db, user._id);
        const averageScore = allQuizzes.length > 0 
          ? allQuizzes.reduce((sum, q) => sum + (q.percentage || 0), 0) / allQuizzes.length 
          : 0;
        ...
        return res.json({
          ...
          totalXpFromQuizzes: allQuizzes.reduce((sum, q) => sum + (q.xpEarned || 0), 0)
        });
      }
